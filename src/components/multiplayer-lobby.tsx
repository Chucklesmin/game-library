...
import { createRoom, getCurrentRoomPlayer, getRoom, joinRoom, type CurrentRoomPlayer } from "@/lib/game-room";
...
export function MultiplayerLobby({ displayName, gameSlug, onRoomChange, onRoleChange }: Props) {
  ...
  return <div className="network-room"><button onClick={() => void create()}>Create Wavelength room</button><form onSubmit={join}><input value={code} onChange={(e) => setCode(e.target.value.replace(/[^a-z0-9]/gi, "").toUpperCase())} placeholder="ROOM CODE" aria-label="Room code" maxLength={6} /><button>Join</button></form><small>{message || "Create a room or join friends with a code."}</small></div>;
}
