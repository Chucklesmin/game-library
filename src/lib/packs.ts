...
const p=(packId:string,x:string)=>(packId==="naughty"?naughtyCards:x).split(/[~\n]+/).map((v,index)=>{const[left,rawRight]=v.split("|").map((part)=>part.trim());const right=rawRight||(left==="Midnight drive"?"Sunrise drive":"Dawn drive");return{id:packId+"-"+String(index+1).padStart(2,"0"),left,right,tags:[packId]}}).filter((card)=>isStrictSpectrumPair(card.left,card.right));
