...
`.trim();
const editorialRewrites:Record<string,[string,string]>={"Plot|Character":["Story-first","Character-first"],"Museum|Concert":["Quiet museum day","Loud concert night"],"Theater|Couch":["Big night out","Cozy night in"],"Jukebox|Algorithm":["Choose every song","Let the feed choose"],"Fiction|Documentary":["Imagined world","Real world"],"Lyrics|Melody":["Words first","Music first"],"Meme|Quote":["Quick joke","Lasting line"],"Cover|Original":["Familiar version","New version"],"Hero|Antihero":["Pure hero","Morally gray"],"Window seat|Aisle seat":["Window view","Easy access"],"Private balcony|Private balcony door":["Open to the view","Closed for privacy"],"Quiet hotel|Thin hotel walls":["Peace and quiet","Can hear everything"],"One bite|One taste":["Tiny sample","Full serving"],"Share a straw|Share a secret":["Publicly sweet","Privately intimate"],"Music low|Music off":["Soft background","Complete silence"],"Lights dim|Lights out":["A little visible","Nothing visible"],"Stay for breakfast|Stay until dinner":["Short visit","All-day visit"],"Friends with chemistry|Friends with benefits":["Keep it platonic","Make it physical"]};
const isStrictSpectrumPair=(left:string,right:string)=>{const a=left.toLowerCase().replace(/[^a-z0-9]+/g," ").trim(),b=right.toLowerCase().replace(/[^a-z0-9]+/g," ").trim(),leftWords=new Set(a.split(" ").filter((word)=>word.length>2)),rightWords=new Set(b.split(" ").filter((word)=>word.length>2));return Boolean(a&&b&&a!==b&&!a.includes(b)&&!b.includes(a)&&![...leftWords].some((word)=>rightWords.has(word)));};
const p=(packId:string,x:string)=>(packId==="naughty"?naughtyCards:x).split(/[~\n]+/).map((v,index)=>{const[left,rawRight]=v.split("|").map((part)=>part.trim());const right=rawRight||(left==="Midnight drive"?"Sunrise drive":"Dawn drive"),rewritten=editorialRewrites[left+"|"+right]??[left,right];return{id:packId+"-"+String(index+1).padStart(2,"0"),left:rewritten[0],right:rewritten[1],tags:[packId]}}).filter((card)=>isStrictSpectrumPair(card.left,card.right));
