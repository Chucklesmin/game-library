...
const isStrictSpectrumPair=(left:string,right:string)=>{const a=left.toLowerCase().replace(/[^a-z0-9]+/g," ").trim(),b=right.toLowerCase().replace(/[^a-z0-9]+/g," ").trim();return Boolean(a&&b&&a!==b&&!a.includes(b)&&!b.includes(a));};
