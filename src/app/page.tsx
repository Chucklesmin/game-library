...
export default function Home() {
  const [phase, setPhase] = useState<Phase>("lobby"), [room, setRoom] = useState(""), [name, setName] = useState("");
  ...
  const roomSubmit = (event: FormEvent) => { event.preventDefault(); if (!room.trim()) return setNotice("Enter a room code to join friends."); setNotice(`Room ${room.toUpperCase()} connects after Supabase setup. Local play is ready now.`); };
  return <main className="shell"><header className="masthead"><a className="brand" href="/">signal<span>.</span></a><div className="tag">A free, original spectrum party game</div><button className="text-button" onClick={() => setPhase("lobby")}>How it works</button></header>
    <section className="game-layout"><aside className="sidebar"><div className="score-card"><p>FIRST TO 12</p><div className={`team amber ${activeTeam === "amber" ? "active" : ""}`}><b>AMBER</b><strong>{scores.amber}</strong></div><div className={`team violet ${activeTeam === "violet" ? "active" : ""}`}><b>VIOLET</b><strong>{scores.violet}</strong></div></div><div className="pack-card"><p>TONIGHT'S FREQUENCY</p><h3>{PACKS[0].name}</h3><span>{allSpectra().length} original spectra across {PACKS.length} packs</span></div></aside>
      ...
      <aside className="room-card"><div className="eyebrow">PLAY REMOTELY</div><h2>Bring your people.</h2><p>Create a private room, share its code, then use video or voice however you like.</p><form onSubmit={roomSubmit}><input value={room} onChange={(event) => setRoom(event.target.value)} placeholder="ROOM CODE" maxLength={8} /><button>Join room</button></form><small>{notice}</small><hr /><h3>Original packs</h3><div className="pack-list">{PACKS.map((pack) => <span key={pack.id}>{pack.emoji} {pack.name}</span>)}</div></aside></section><footer>Signal is an independent, non-commercial game. No accounts, purchases, or tracking required for local play.</footer></main>;
}
